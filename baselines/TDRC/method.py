...
class Model(object):
    ...
    def TDRC(self, X, S_d, S_m, r=4, alpha=0.125, beta=0.25, lam=0.001, tol=1e-6, max_iter=500):
        ...
        rho_2 = 1
        np.random.seed(0)
        C = np.asmatrix(np.random.rand(m, r))
        P = np.asmatrix(np.random.rand(d, r))
        D = np.asmatrix(np.random.rand(t, r))
        ...
        for i in range(max_iter):
            G = np.asmatrix(tl.tenalg.khatri_rao([P, D]))
            ...
            U = np.asmatrix(tl.tenalg.khatri_rao([C, D]))
            Z = M_3 * W.T
            ...
            B = np.asmatrix(tl.tenalg.khatri_rao([C, P]))
        ...
        predict_X = np.array(tl.fold(np.array(C * np.asmatrix(tl.tenalg.khatri_rao([P, D])).T), 0, X.shape))
    ...
    def TFAI_CP_within_mod(self, X, S_m, S_d, r=3, alpha=0.25, beta=1.0, lam=0.1, tol=1e-7, max_iter=500, seed=0):
        ...
        for i in range(max_iter):
            ...
            C = np.asmatrix(
                sp.linalg.solve_sylvester(np.array(alpha * L_C + lam * np.asmatrix(np.eye(m))), np.array(G.T * G), X_1 * G))
            U = np.asmatrix(tl.tenalg.khatri_rao([C, D]))
            P = np.asmatrix(
                sp.linalg.solve_sylvester(np.array(beta * L_P + lam * np.asmatrix(np.eye(d))), np.array(U.T * U), X_2 * U))
            B = np.asmatrix(tl.tenalg.khatri_rao([C, P]))
            D = X_3 * B * np.linalg.inv(B.T * B + lam * np.eye(r))
            ...
            if err < tol:
                print(i)
                break
        predict_X = tl.fold(np.array(C * np.asmatrix(tl.tenalg.khatri_rao([P, D])).T), 0, X.shape)
        return predict_X
