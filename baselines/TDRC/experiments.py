...
class Experiments(object):
    ...
    def CV_type(self):
        ...
        for k in range(k_folds):
            ...
            miRNA_func_similarity_matrix = np.asmatrix(self.mir_dis_data.get_functional_sim(train_matrix))
            ...
            for t in range(sample_num_eval):
                ...
                predict_score = np.asmatrix(predict_matrix.flatten())
                real_matrix = self.mir_dis_data.type_tensor[test_index[0][t], test_index[1][t]]
                real_score = np.asmatrix(real_matrix.flatten())
    ...
    def cv_tensor_model_evaluate(self, association_tensor, predict_tensor, train_index, seed):
        ...
        real_score = np.column_stack(
            (np.asmatrix(association_tensor[test_ne_index].flatten()), np.asmatrix(association_tensor[train_index].flatten())))
        predict_score = np.column_stack(
            (np.asmatrix(predict_tensor[test_ne_index].flatten()), np.asmatrix(predict_tensor[train_index].flatten())))
