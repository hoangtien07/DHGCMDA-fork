...
class Hyper_Model(object):
    ...
    def SPLDHyperAWNTF(self, X, m_embeding, d_embeding, W, r=4, alpha = 2, beta = 2, lam_t = 0.001, lam_c = 0.3, max_iter=2000, tol=1e-5):
        ...
        np.random.seed(0)
        M = np.asmatrix(np.random.rand(m, r))
        D = np.asmatrix(np.random.rand(d, r))
        T = np.asmatrix(np.random.rand(t, r))
        ...
        X_1 = np.asmatrix(tl.unfold(X, 0))
        X_2 = np.asmatrix(tl.unfold(X, 1))
        X_3 = np.asmatrix(tl.unfold(X, 2))

        '''adaptive weight tensor'''
        np.random.seed(1)
        C_1 = np.asmatrix(np.random.rand(X_1.shape[0],X_1.shape[1]))
        C_2 = np.asmatrix(np.random.rand(X_2.shape[0],X_2.shape[1]))
        C_3 = np.asmatrix(np.random.rand(X_3.shape[0],X_3.shape[1]))


        I_1 = np.asmatrix(np.ones(X_1.shape))
        I_2 = np.asmatrix(np.ones(X_2.shape))
        I_3 = np.asmatrix(np.ones(X_3.shape))


        W_1 = np.asmatrix(tl.unfold(W, 0))
        W_2 = np.asmatrix(tl.unfold(W, 1))
        W_3 = np.asmatrix(tl.unfold(W, 2))
        ...
        while k > k_end:
            for i in range(max_iter):
                A = np.asmatrix(tl.tenalg.khatri_rao([D, T]))
                ...
                B = np.asmatrix(tl.tenalg.khatri_rao([M, T]))
                ...
                E = np.asmatrix(tl.tenalg.khatri_rao([M, D]))
            ...
            output_X = tl.fold(np.array(T * E.T), 2, X.shape)
            New_X_1 = np.asmatrix(tl.unfold(output_X, 0))
            New_X_2 = np.asmatrix(tl.unfold(output_X, 1))
            New_X_3 = np.asmatrix(tl.unfold(output_X, 2))
        ...
        predict_X = np.array(tl.fold(np.array(M * np.asmatrix(tl.tenalg.khatri_rao([D, T])).T), 0, X.shape))
