...
class Experiments(object):
    ...
    def CV_type(self):
        ...
        for k in range(k_folds):
            ...
            miRNA_func_similarity_matrix = np.asmatrix(self.mir_dis_data.mi_sim)
            dis_sen_similarity_matrix = np.asmatrix(self.mir_dis_data.dis_sim)
            ...
            concat_miRNA = np.asmatrix(np.hstack([temp_association_matrix,IM]))
            concat_disease = np.asmatrix(np.hstack([temp_association_matrix.T, ID]))
